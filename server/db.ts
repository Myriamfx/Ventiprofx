...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    ...
    // passwordHash is handled separately because we need to allow null values
    if (user.passwordHash !== undefined) {
      const normalized = user.passwordHash ?? null;
      values.passwordHash = normalized;
      updateSet.passwordHash = normalized;
    }
